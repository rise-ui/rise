...
impl WindowOptions {
    pub fn get_window_position(&self, monitor: glutin::MonitorId) -> (i32, i32) {
        ...
        let monitor_size = (
            (monitor_size.width / hidpi) as f32,
            (monitor_size.height / hidpi) as f32
        );
...
impl App {
    pub fn new(options: WindowOptions, dom: DOMTree<BasicEvent>) -> App {
        let window_builder = WindowBuilder::new()
            .with_dimensions((options.window_size.0, options.window_size.1).into())
        ...
        window.window.set_position((position.0, position.1).into());

        let webrender_context = WebRenderContext::new(&mut window, &event_loop);
        let event_loop = Rc::new(RefCell::new(event_loop));
        let ui = Ui::new(event_loop.clone(), webrender_context, dom, window);
